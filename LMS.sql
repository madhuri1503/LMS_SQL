...
('B001', 'M101', '123 Main St', '+91784653758'),
('B002', 'M102', '456 Elm St', '+918497238573'),
('B003', 'M103', '789 Oak St', '+919086492674'),
...
('E105', 'Sarah Brown', 'Librarian', 42000.00),
...
# 11.Retrieve the names and addresses of customers who have borrowed more than one book.
...
GROUP BY customer.Customer_name, customer.Customer_address
HAVING COUNT(issuestatus.Issue_Id) > 1;

#12.Find the total rental price of all books borrowed by each customer, listing the customers name and the total amount.
SELECT customer.Customer_name, SUM(books.Rental_Price) AS Total_Rental
FROM customer
INNER JOIN issuestatus ON customer.Customer_Id = issuestatus.Issued_cust
INNER JOIN books ON issuestatus.Isbn_book = books.ISBN
GROUP BY customer.Customer_name;

#13. Retrieve the top 3 highest paid employees along with their branch information.
SELECT employee.Emp_name, employee.Salary, branch.Branch_no, branch.Branch_address
FROM employee
INNER JOIN branch ON employee.branch_no = branch.Branch_no
ORDER BY employee.Salary DESC
LIMIT 3;

#14. Find the books that have never been borrowed, listing their titles and authors.
SELECT books.Book_title, books.Author
FROM books
LEFT JOIN issuestatus ON books.ISBN = issuestatus.Isbn_book
WHERE issuestatus.Isbn_book IS NULL;
